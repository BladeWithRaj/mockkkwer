      ...
      <div class="profile-page container page-enter">
        ...
        <div class="profile-header-card animate-fadeInUp">
          ${window.FirebaseAuth && FirebaseAuth.isLoggedIn() && FirebaseAuth.getUser()?.photoURL
            ? `<img src="${FirebaseAuth.getUser().photoURL}" alt="" style="width:64px;height:64px;border-radius:50%;border:3px solid rgba(99,102,241,0.4);object-fit:cover;" />`
            : `<div class="profile-avatar">${avatarHTML}</div>`
          }
          <div class="profile-name">${userName}</div>
          ${window.FirebaseAuth && FirebaseAuth.isLoggedIn() && FirebaseAuth.getUser()?.email
            ? `<div style="font-size:13px;color:#64748b;margin-top:-4px;">${FirebaseAuth.getUser().email}</div>`
            : ''
          }
          <div class="profile-level">${tier.icon} ${tier.title} — Tier ${tier.level}/5</div>

          ${!window.FirebaseAuth || !FirebaseAuth.isLoggedIn() ? `
          <div class="fa-profile-login">
            <p>🔒 Sign in to save your progress across devices</p>
            <button class="fa-google-btn" onclick="FirebaseAuth.signInWithGoogle()">
